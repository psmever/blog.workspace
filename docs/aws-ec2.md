...
- `backend`와 `frontend`는 서로 독립된 배포 단위이며, 서버에서 `origin/develop` 을 `main` 에 `--no-ff` 로 병합하고 `origin/main` 으로 push한 뒤 배포합니다.
...
- 신규 커밋 승격 배포가 성공하면 서버가 앱 저장소 `origin` 에 `deploy/prod/<app>/<timestamp>` annotated tag를 자동 push 합니다.
...
각 remote는 서버 전용 SSH Deploy Key를 사용하고 fetch, `main` push, tag push 권한을 가져야 합니다. backend/frontend는 저장소별로 서로 다른 Deploy Key를 사용합니다. 병합 커밋과 annotated tag 작성을 위해 각 서버 저장소에 `user.name`, `user.email`도 설정합니다.
...
- 배포 전 서버에서 `origin/develop` 을 `main` 에 `--no-ff` 로 병합하고 `origin/main` 으로 push합니다.
- 서버 배포 기준은 두 저장소 모두 `origin/main` 입니다.
- `origin/develop` 에 신규 커밋이 없으면 병합, push, 배포 태그 생성을 생략하고 현재 `main` 을 재배포합니다.
...
- backend 신규 승격 배포 성공 시 서버 `blog.backend` 에 `deploy/prod/backend/<timestamp>` 태그를 생성하고 `origin` 으로 push 합니다.
- frontend 신규 승격 배포 성공 시 서버 `blog.frontend` 에 `deploy/prod/frontend/<timestamp>` 태그를 생성하고 `origin` 으로 push 합니다.
